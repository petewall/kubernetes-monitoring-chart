...
![Version: 0.0.8](https://img.shields.io/badge/Version-0.0.8-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.1.6](https://img.shields.io/badge/AppVersion-1.1.6-informational?style=flat-square)
...
The metrics, pod logs, and cluster events that are captured are then sent to Prometheus and Loki services.
This chart does not deploy Prometheus or Loki, but it can be configured to send data to services hosted on the same cluster, or remotely (e.g. on Grafana Cloud).
...
If you want to add configuration to the Grafana Agent config generated by this Helm chart, you have a few options:

### Additional Flow configuration

You can add any additional Flow components to the generated ConfigMap by using the `extraConfig` value.
Anything there will be appended to the config file.
You can utilize any existing components in the config, and some that might be useful are:

* `discovery.kubernetes "nodes"`
* `discovery.kubernetes "pods"`
* `discovery.kubernetes "services"`

Which should already be present to discover the respective components on your cluster.

### Using Prometheus Operator PodMonitors and ServiceMonitors

This helm chart configures the Grafana Agent to detect and use PodMonitors and ServiceMonitors to scrape metrics.
